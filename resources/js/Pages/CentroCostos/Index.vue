...
const serverSearch = debounce(() => {
    data.params.columnFilters = pickBy(data.columnFilters);
        ...
        <div class="py-6 px-4 sm:px-6 lg:px-8">
            <div class="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                ...
                <div class="flex items-center gap-3">
                    <PrimaryButton v-if="can(['isSuper'])" class="shadow-lg shadow-primary/20 transition-all hover:scale-105" @click="data.createOpen = true">
                        <PlusIcon class="w-4 h-4 mr-1" />
                        {{ lang().button.add }}
                    </PrimaryButton>
                    <Create :show="data.createOpen" @close="data.createOpen = false" :listaSupervisores="props.listaSupervisores" :title="props.title" />
                    <Edit :show="data.editOpen" @close="data.editOpen = false" :listaSupervisores="props.listaSupervisores" :CentroCosto="data.generico" :losSelect="props.losSelect" :title="props.title" />
                    <Delete :show="data.deleteOpen" @close="data.deleteOpen = false" :CentroCosto="data.generico" :title="props.title" />
                    <CostDetailsModal :show="data.costDetailsOpen" @close="data.costDetailsOpen = false" :centroCosto="data.generico" />
                </div>
            </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead class="sticky top-0 z-20 bg-white dark:bg-gray-800 shadow-sm">
                            <tr class="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider border-b border-gray-100 dark:border-gray-700">
                                <th v-for="(titulos, indiceN) in nombresTabla[0]" :key="indiceN" @click="order(nombresTabla[1][indiceN])" class="px-4 py-3 cursor-pointer hover:text-amber-600 transition-colors">
                                    <div class="flex items-center gap-1">
                                        {{ titulos }}
                                        <ChevronUpDownIcon v-if="nombresTabla[1][indiceN]" class="w-4 h-4 text-gray-400" />
                                    </div>
                                </th>
                            </tr>
                             <tr class="bg-gray-50 dark:bg-gray-900/50">
                                <th v-for="(field, index) in nombresTabla[1]" :key="index" class="px-2 py-1.5">
                                    <TextInput
                                        ...
                                        v-model="data.columnFilters[field]"
                                        type="text"
                                    ...
                                    <v-select
                                        v-if="field === 'Zouna'"
                                        v-model="data.columnFilters[field]"
                                        :options="props.losSelect['zona'].map(z => z.label)"
                                        placeholder="Filtrar..."
                                        class="custom-v-select-sm"
                                    ></v-select>
                                    <TextInput
                                        v-if="field === 'supervisores'"
                                        v-model="data.columnFilters[field]"
                                        type="text"
                                        class="h-8 text-xs rounded-lg border-gray-200 focus:ring-amber-500 w-full"
                                        placeholder="Filtrar..."
                                    />
                                     <v-select
                                        v-if="field === 'activo' || field === 'ValidoParaFacturar'"
                                        v-model="data.columnFilters[field]"
                                        :options="['Si', 'No']"
                                        placeholder="Filtrar..."
                                        class="custom-v-select-sm"
                        ...
                        <tbody class="divide-y divide-gray-50 dark:divide-gray-700/50">
                            <tr v-for="(clasegenerica, index) in clientSideFilteredData" :key="clasegenerica.id" class="group hover:bg-amber-50/30 dark:hover:bg-amber-900/5 transition-colors">
                                <td v-if="can(['update centroCostos'])" class="px-4 py-4 whitespace-nowrap">
                                    <div class="flex items-center gap-1 opacity-60 group-hover:opacity-100 transition-opacity">
                                        <button @click="(data.costDetailsOpen = true), (data.generico = clasegenerica)" class="p-1.5 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 rounded-lg hover:bg-green-500 hover:text-white transition-all transform hover:scale-110" v-tooltip="'Ver Detalles de Costos'">
...
<!--                                        <Link :href="route('CentroCostos.show', clasegenerica)" class="p-1.5 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 rounded-lg hover:bg-blue-500 hover:text-white transition-all transform hover:scale-110" v-tooltip="lang().tooltip.see"><EyeIcon class="w-4 h-4" /></Link>-->
                                        <Link :href="route('CentroCostos.table', clasegenerica.id)" class="p-1.5 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 rounded-lg hover:bg-emerald-500 hover:text-white transition-all transform hover:scale-110" v-tooltip="lang().tooltip.Reporte"><DocumentIcon class="w-4 h-4" /></Link>
                                        <button @click="(data.editOpen = true), (data.generico = clasegenerica)" class="p-1.5 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 rounded-lg hover:bg-amber-500 hover:text-white transition-all transform hover:scale-110" v-tooltip="lang().tooltip.edit"><PencilIcon class="w-4 h-4" /></button>
                                        <button v-show="can(['delete centroCostos'])" @click="(data.deleteOpen = true), (data.generico = clasegenerica)" class="p-1.5 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 rounded-lg hover:bg-red-500 hover:text-white transition-all transform hover:scale-110" v-tooltip="lang().tooltip.delete"><TrashIcon class="w-4 h-4" /></button>
                                    </div>
                                </td>
                                <td class="px-4 py-4 text-sm text-gray-500 dark:text-gray-400 font-medium">#{{ index + 1 }}</td>
                                <td class="px-4 py-4">
                                    <div class="text-sm font-bold text-gray-900 dark:text-white">{{ clasegenerica.nombre }}</div>
                                    <div class="text-xs text-gray-500 dark:text-gray-400 max-w-[200px] truncate" :title="clasegenerica.descripcion">{{ clasegenerica.descripcion }}</div>
                                </td>
                                <td v-show="can(['update centroCostos'])" class="px-4 py-4 whitespace-nowrap text-sm font-semibold text-amber-600 dark:text-amber-400">{{ formatPesosCol(clasegenerica.mano_obra_estimada) }}</td>
                                <td v-show="can(['update centroCostos'])" class="px-4 py-4 whitespace-nowrap">
                                    <span class="px-2 py-1 text-xs font-medium bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 rounded-md">{{ clasegenerica.zona ? clasegenerica.zona.nombre : 'N/A' }}</span>
                                </td>
                                <td v-show="can(['update centroCostos'])" class="px-4 py-4">
                                    <div v-if="clasegenerica.users && clasegenerica.users.length" class="flex flex-wrap gap-1">
                                        <span v-for="user in clasegenerica.users" :key="user.id" class="text-[10px] px-1.5 py-0.5 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-lg border border-blue-100 dark:border-blue-800">{{ user.name.trim() }}</span>
                                    </div>
                                    <span v-else class="text-[10px] text-gray-400 italic">Sin supervisor</span>
                                </td>
                                <td v-show="can(['update centroCostos'])" class="px-4 py-4 whitespace-nowrap text-center">
                                    <span :class="clasegenerica.activo ? 'text-emerald-500 bg-emerald-50 dark:bg-emerald-500/10' : 'text-red-500 bg-red-50 dark:bg-red-500/10'" class="p-1 px-2 rounded-full text-lg leading-none inline-block">{{ clasegenerica.activo ? '●' : '○' }}</span>
                                </td>
                                <td v-show="can(['update centroCostos'])" class="px-4 py-4 whitespace-nowrap text-center">
                                    <div :class="clasegenerica.ValidoParaFacturar ? 'text-blue-500 bg-blue-50 dark:bg-blue-500/10' : 'text-gray-400 bg-gray-50 dark:bg-gray-500/10'" class="p-1 px-2 rounded-full text-xs font-bold inline-block border border-current">{{ clasegenerica.ValidoParaFacturar ? 'FACTURABLE' : 'INTERNO' }}</div>
                                </td>
                                <td v-show="can(['update centroCostos'])" class="px-4 py-4 whitespace-nowrap text-xs font-mono text-gray-400">{{ clasegenerica.clasificacion }}</td>
                            </tr>
                        </tbody>
                    </table>
