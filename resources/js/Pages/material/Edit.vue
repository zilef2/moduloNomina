...
watchEffect(() => {
    if (props.show) {
        ...
        //     form[element] =  props.materiala[element]
        // });
        form.errors = {}
        props.titulos.forEach(names => {
            form[names['order']] = props.materiala[names['order']]
        });

        // form.codigo = props.materiala?.codigo
    }
})

const update = () => {
    form.put(route('material.update', props.materiala?.id), {
        preserveScroll: true,
        onSuccess: () => {
            emit("close")
            form.reset()
