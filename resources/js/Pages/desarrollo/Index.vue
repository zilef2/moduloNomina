                    ...
                    <div class="container px-5 py-24 mx-auto">
                        <div class="my-2 divide-y-2 divide-gray-100 rounded-2xl">
                            <div v-for="(claseFromController, indexu) in props.fromController.data" :key="indexu"
                                ...
                                <div
                                    class="md:w-64 md:mb-0 mb-6 flex-shrink-0 flex flex-col hover:bg-indigo-50 border-r-2 border-gray-200 mx-2">
                                    <span class="font-semibold title-font text-gray-700">{{
                                            claseFromController.nombre
                                        }}</span>
                                    <span
                                        class="mt-1 text-gray-500 text-sm">Reunión: {{ claseFromController.fecha_reunion }}</span>
                                ...
                                <div class="w-1/2 hover:bg-indigo-50 border-r-2 border-gray-200 mx-2">
                                    <h2 class="text-2xl font-medium text-gray-900 title-font mb-2">
                                        <b>Etapa {{ obtenerIndice(claseFromController.estado) }}:
                                            {{ claseFromController.estado }}</b>
                                    </h2>
                                ...
                                <div class="md:flex-grow hover:bg-indigo-50 border-r-2 border-gray-200 mx-2">
                                    <p class="leading-relaxed">Valor inicial:
                                        {{ formatPesosCol(claseFromController.valor_inicial) }}</p>
                                    <p class="leading-relaxed">Primer pago acordado:
                                        <b>{{ formatPesosCol(claseFromController.valor_parcial1) }}</b></p>
                                    <p class="leading-relaxed"> Pagos: {{ claseFromController.valorino }}</p>
                                    <p class="leading-relaxed"> Pagado:
                                        {{ formatPesosCol(claseFromController.totalpagado) }}</p>
                                    <p class="leading-relaxed"> Deuda:
                                        {{ formatPesosCol(claseFromController.Deudau) }}</p>
                                    <p v-if="claseFromController.fecha_cotizacion_aceptada"
                                       class="text-indigo-500 inline-flex items-center mt-4">
                                        Cotizacion aceptada el: {{ claseFromController.fecha_cotizacion_aceptada }}
                                    </p>
                                    <p class="text-indigo-500 inline-flex items-center mt-4 cursor-pointer">
                                        <a class="inline-flex px-8 py-4 rounded-lg text-white font-semibold text-lg overflow-hidden 
