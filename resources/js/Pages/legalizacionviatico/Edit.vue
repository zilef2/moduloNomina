...
watchEffect(() => {
    if (props.show) {
        ...
        //     form[element] =  props.legalizacionviaticoa[element]
        // });
        form.errors = {}
        props.titulos.forEach(names => {
            form[names['order']] = props.legalizacionviaticoa[names['order']]
        });

        // form.codigo = props.legalizacionviaticoa?.codigo
    }
})

const update = () => {
    form.put(route('legalizacionviatico.update', props.legalizacionviaticoa?.id), {
        preserveScroll: true,
        onSuccess: () => {
            emit("close")
            form.reset()
