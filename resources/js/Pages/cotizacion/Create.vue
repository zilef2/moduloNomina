...
function ValidarVaciosAIU() {
  console.log("=>(Create.vue:194) noValidarAIU", noValidarAIU);
  let result = true
  printForm.forEach(element => {
...
const create = () => {
  let esCorrecto
  console.log("=>(Create.vue:208) form.modoaiu", form.modoaiu);
  if (form.modoaiu) {
      ...
      <form class="px-6 pt-4 pb-48 m-4 2xl:m-10" @submit.prevent="create">
        ...
        <div :class="clasedelgrid">
          <div class="rounded-xl"><label name="numero_cot"> {{ lang().label.numero_cot }} </label>
            <TextInput v-model="form.numero_cot" :error="form.errors.numero_cot" :placeholder="lang().label.numero_cot"
