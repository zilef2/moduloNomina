...
watchEffect(() => {
    if (props.show) {
        ...
        //     form[element] =  props.pagodesarrolloa[element]
        // });
        form.errors = {}
        props.titulos.forEach(names => {
            form[names['order']] = props.pagodesarrolloa[names['order']]
        });

        // form.codigo = props.pagodesarrolloa?.codigo
    }
})

const update = () => {
    form.put(route('pagodesarrollo.update', props.pagodesarrolloa?.id), {
        preserveScroll: true,
        onSuccess: () => {
            emit("close")
            form.reset()
