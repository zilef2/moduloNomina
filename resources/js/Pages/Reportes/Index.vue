    ...
    const selectAll = (event) => {
        if (event.target.checked === false) {
            ...
            props.reports?.data.forEach((report) => {
                data.selectedId.push(report.id)
            })
        }
    }
    const select = () => {
        if (props.reports?.data.length == data.selectedId.length) {
        ...
        <div class="space-y-4">
            ...
            <div class="relative bg-white dark:bg-gray-800 shadow sm:rounded-lg">
                    ...
                    <table class="w-full">
                        <thead class="uppercase text-sm border-t border-gray-200 dark:border-gray-700">
                            <tr class="dark:bg-gray-900/50 text-left">
                                ...
                                <th v-for="(titulos,indiceN) in nombresTabla[0]" :key="indiceN" class="px-2 py-4 cursor-pointer" v-on:click="order( nombresTabla[1][indiceN] )">
                        ...
                        <tbody>
                            <tr v-for="(report, index) in reports.data" :key="index"
                                ...
                                <td class="whitespace-nowrap py-4 px-2 sm:py-3 text-center">
                                    <input type="checkbox" @change="select" :value="report.id"
                                        v-model="data.selectedId"
                                        class="rounded dark:bg-gray-900 border-gray-300 dark:border-gray-700 text-primary dark:text-primary shadow-sm focus:ring-primary/80 dark:focus:ring-primary dark:focus:ring-offset-gray-800 dark:checked:bg-primary dark:checked:border-primary"
                                         />
                                </td>
                                <td class="whitespace-nowrap py-4 px-2 sm:py-3 text-center">{{ ++index }}</td>
                                <td class="whitespace-nowrap py-4 px-2 sm:py-3">{{ formatDate(report.hora_inicio) }}</td>
                                <td class="whitespace-nowrap py-4 px-2 sm:py-3">{{ formatDate(report.hora_fin) }}</td>
                                <td class="whitespace-nowrap py-4 px-2 sm:py-3">{{ report.centro_compra_id }}</td>
                                <td class="whitespace-nowrap py-4 px-1 sm:py-3">{{ report.observaciones }} </td>
                                <!-- <td class="whitespace-nowrap py-4 px-2 sm:py-3">{{ report.created_at }}</td>
                                <td class="whitespace-nowrap py-4 px-2 sm:py-3">{{ report.updated_at }}</td> -->
                                <td class="whitespace-nowrap py-4 px-2 sm:py-3">
                                    <div class="flex justify-center items-center">
                                        <div class="rounded-md overflow-hidden">
                                            <InfoButton type="button"
                                                @click="(data.editOpen = true), (data.report = report)"
                                                class="px-2 py-1.5 rounded-none" v-tooltip="lang().tooltip.edit">
                                                <PencilIcon class="w-4 h-4" />
                                            </InfoButton>
                                            <DangerButton type="button"
                                                @click="(data.deleteOpen = true), (data.report = report)"
                ...
                <div class="flex justify-between items-center p-2 border-t border-gray-200 dark:border-gray-700">
                    <Pagination :links="props.reports" :filters="data.params" />
                </div>
            </div>
        </div>
    </AuthenticatedLayout>
