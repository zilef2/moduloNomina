...
import Create from '@/Pages/consignarViatico/Create.vue';
import Edit from '@/Pages/consignarViatico/Edit.vue';
import Delete from '@/Pages/consignarViatico/Delete.vue';
...
watch(() => _.cloneDeep(data.params), debounce(() => {
    ...
    router.get(route("consignarViatico.index"), params, {
        replace: true,
        preserveState: true,
        preserveScroll: true,
    })
...
const selectAll = (event) => {
    if (event.target.checked === false) {
        ...
        props.fromController?.data.forEach((consignarViatico) => {
            data.selectedId.push(consignarViatico.id)
        })
    }
}
            ...
            <div class="px-4 sm:px-0">
                <div class="rounded-lg overflow-hidden w-fit">
                    <PrimaryButton class="rounded-none" @click="data.createOpen = true"
                        v-if="can(['create consignarViatico'])">
                        {{ lang().button.new }}
                    </PrimaryButton>

                    <Create v-if="can(['create consignarViatico'])" :numberPermissions="props.numberPermissions"
                        :titulos="titulos" :show="data.createOpen" @close="data.createOpen = false" :title="props.title"
                        :losSelect=props.losSelect />

                    <Edit v-if="can(['update consignarViatico'])" :titulos="titulos"
                        :numberPermissions="props.numberPermissions" :show="data.editOpen" @close="data.editOpen = false"
                        :consignarViaticoa="data.consignarViaticoo" :title="props.title" :losSelect=props.losSelect />

                    <Delete v-if="can(['delete consignarViatico'])" :numberPermissions="props.numberPermissions"
                        :show="data.deleteOpen" @close="data.deleteOpen = false" :consignarViaticoa="data.consignarViaticoo"
                        :title="props.title" />
                </div>
            </div>
                ...
                <div class="flex justify-between p-2">
                    <div class="flex space-x-2">
                        ...
                        <!-- <DangerButton @click="data.deleteBulkOpen = true"
                            v-show="data.selectedId.length != 0 && can(['delete consignarViatico'])" class="px-3 py-1.5"
                                ...
                                <td v-if="numberPermissions > 1" class="whitespace-nowrap py-4 w-12 px-2 sm:py-3">
                                    <div class="flex justify-center items-center">
                                        <div class="rounded-md overflow-hidden">
                                            <InfoButton v-show="can(['update consignarViatico'])" type="button"
                                                @click="(data.editOpen = true), (data.consignarViaticoo = claseFromController)"
                                                class="px-2 py-1.5 rounded-none" v-tooltip="lang().tooltip.edit">
                                                <PencilIcon class="w-4 h-4" />
                                            </InfoButton>
                                            <DangerButton v-show="can(['delete consignarViatico'])" type="button"
                                                @click="(data.deleteOpen = true), (data.consignarViaticoo = claseFromController)"
