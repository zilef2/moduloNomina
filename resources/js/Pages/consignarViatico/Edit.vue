...
watchEffect(() => {
    if (props.show) {
        ...
        //     form[element] =  props.consignarViaticoa[element]
        // });
        form.errors = {}
        props.titulos.forEach(names => {
            form[names['order']] = props.consignarViaticoa[names['order']]
        });

        // form.codigo = props.consignarViaticoa?.codigo
    }
})

const update = () => {
    form.put(route('consignarViatico.update', props.consignarViaticoa?.id), {
        preserveScroll: true,
        onSuccess: () => {
            emit("close")
            form.reset()
