...
watchEffect(() => {
    if (props.show) {
        ...
        //     form[element] =  props.solicitud_viaticoa[element]
        // });
        form.errors = {}
        props.titulos.forEach(names => {
            form[names['order']] = props.solicitud_viaticoa[names['order']]
        });

        // form.codigo = props.solicitud_viaticoa?.codigo
    }
})

const update = () => {
    form.put(route('solicitud_viatico.update', props.solicitud_viaticoa?.id), {
        preserveScroll: true,
        onSuccess: () => {
            emit("close")
            form.reset()
