...
const destory = () => {
    form.delete(route('solicitud_viatico.destroy', props.solicitud_viaticoa?.id), {
        preserveScroll: true,
        onSuccess: () => {
            emit("close")
            form.reset()
        ...
        <Modal :show="props.show" @close="emit('close')" :maxWidth="'lg'">
            <form class="p-6" @submit.prevent="destory">
                ...
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                    {{ lang().label.delete_confirm }} la solicitud de <b>{{ props.solicitud_viaticoa.Solicitante }}</b>?
                </p>
