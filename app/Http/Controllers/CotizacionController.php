...
class CotizacionController extends Controller
{
	...
	public function Filtros(&$cotizacions, $request)
	{
		...
		if ($request->has('search6')) {
			$cotizacions = $cotizacions->where("tipo", $request->search6['value']);
		}
		...
		if ($request->has('columnFilters')) {
			...
			foreach ($filters as $column => $value) {
				if (!empty($value)) {
					// Handle foreign key: centro_costo_id
					if ($column === 'centro_costo_id') {
						$cotizacions = $cotizacions->whereHas('centro', function ($q) use ($value) {
							$q->where('nombre', 'LIKE', "%{$value}%");
						});
					}
					// Handle zona (Zouna in display)
					elseif ($column === 'Zouna') {
						$cotizacions = $cotizacions->whereHas('zona', function ($q) use ($value) {
							$q->where('nombre', 'LIKE', "%{$value}%");
						});
					}
					elseif ($column === 'Prealiza') {
						$cotizacions = $cotizacions->whereHas('prealiza', function ($q) use ($value) {
							$q->where('name', 'LIKE', "%{$value}%");
						});
					}
					// Standard columns
					else {
						$cotizacions = $cotizacions->where($column, 'LIKE', "%{$value}%");
					}
				}
			}
		}
	...
	public function Dependencias2025(): array
	{ //todo: torescue:
